### dp的定义
看到target长度为15，经典的状态压缩。dp定义应该基于target子集而不是字符出现的次数（后者会爆空间）

### 一些进一步讨论
另外是用dp[index][mask]还是dp[mask] + 记忆化搜索，这里我估计是dp[mask] + 记忆化搜索更好，因为dp[index][mask]会有很多无效的状态
（两种方式比较：前者转移更快但是需要求解所有状态，尽管很多可能并没有出现在ans对应的那个状态的转移路径上）
